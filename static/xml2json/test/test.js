...
describe("xml2json", function() {
    ...
    it("converts with forceArrays", function(done) {
        ...
        var result = parser.toJson(xml, {
            arrayNotation: ["drivers", "vehicles"]
        });
    ...
    describe("coercion", function() {
        ...
        it("works with coercion", function(done) {
            ...
            expect(result.itemRecord.value[1].stringValue.number).to.equal(
                false
            );
            expect(result.itemRecord.value[2].moneyValue.number).to.equal(true);
            expect(result.itemRecord.value[2].moneyValue["$t"]).to.equal(
                104.95
            );
        ...
        it("works without coercion", function(done) {
            ...
            expect(result.itemRecord.value[0].longValue["$t"]).to.equal(
                "12345"
            );
            expect(result.itemRecord.value[1].stringValue.number).to.equal(
                "false"
            );
            expect(result.itemRecord.value[2].moneyValue.number).to.equal(
                "true"
            );
            expect(result.itemRecord.value[2].moneyValue["$t"]).to.equal(
                "104.95"
            );
            expect(result.itemRecord.value[2].moneyValue.text).to.equal(
                "123.45"
            );
            expect(result.itemRecord.value[8].text["$t"]).to.equal("42.42");
            done();
...
describe("json2xml", function() {
    ...
    it("works with array notation", function(done) {
        ...
        var expectedJson = JSON.parse(
            internals.readFixture("array-notation.json")
        );
